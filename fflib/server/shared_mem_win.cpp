...
int SharedSyncmemMgr::init_master(int shared_key)
{
    ...
    //m_worker_shmid = //!TODO mget((key_t)shared_key, sizeof(MasterSharedData) + sizeof(struct wroker_shared_data_t)*m_worker_num, 0666);
    ...
    {
        //m_worker_shmid = //!TODO mget((key_t)shared_key, sizeof(MasterSharedData) + sizeof(struct wroker_shared_data_t)*m_worker_num, 0666|IPC_CREAT);
        if(m_worker_shmid == -1)
        {
            if (EINVAL == errno)//!size 变大了,需要先删除老的
            {
                printf("sharedmem realloc\n");
                int oldid = 0;//!TODO mget((key_t)shared_key,  1, 0666);
                if (oldid != - 1){
                    //!TODO mctl(oldid, IPC_RMID, NULL);
                    //m_worker_shmid = //!TODO mget((key_t)shared_key, sizeof(MasterSharedData) + sizeof(struct wroker_shared_data_t)*m_worker_num, 0666|IPC_CREAT);
                }
            }
            if(m_worker_shmid == -1)
    ...
    void* p = NULL;//!TODO shmat(m_worker_shmid, 0, 0);
...
int SharedSyncmemMgr::init_worker(int shared_key, int work_index_, TaskQueue* tq ){
    //!TODO m_worker_shmid = shmget((key_t)shared_key, sizeof(MasterSharedData) + sizeof(struct wroker_shared_data_t)*m_worker_num, 0666);
    if(m_worker_shmid == -1)
    {
...
bool MasterSharedData::timelock(int sec)
{
    ...
    if (pthread_mutex_timedlock(&master_lock, &outtime))
    {
        return false;
    }
    return true;
