...
#define GEN_strServiceName(M, X, Y) snprintf(M, sizeof(M), "%s@%u", X, Y)
...
FFSlot::FFCallBack* FFRpcOps::genCallBack(R (*func_)(RPCReq<IN_T, OUT_T>&))
{
    struct lambda_cb: public FFSlot::FFCallBack
    {
        ...
        virtual void exe(FFSlot::CallBackArg* args_)
        {
            ...
            if (msg_data->errinfo.empty())
            {
                MsgTool::decode(req.msg, msg_data->body, &(req.errinfo));
            }
            else
            {
                req.errinfo = msg_data->errinfo;
            }
            req.destNodeId = msg_data->destNodeId;
            req.callbackId = msg_data->callbackId;
