...
#define MYSQL_INVALID_SOCKET mysql_SocketInvalid()
...
)
{
  ...
  if (socket.m_psi != NULL)
    PSI_SOCKET_CALL(set_SocketInfo)(socket.m_psi, NULL, addr, addr_len);
#endif
}
  ...
  MYSQL_SOCKET mysql_socket, const struct sockaddr *addr, socklen_t len)
{
  ...
  if (mysql_socket.m_psi != NULL)
  {
    ...
    /* Instrumentation end */
    if (result == 0)
      PSI_SOCKET_CALL(set_SocketInfo)(mysql_socket.m_psi, NULL, addr, len);
