...
struct st_mysql_mutex
{
  ...
  /**
    ...
    for binary compatibility of the @c mysql_Mutex interface.
  */
...
/**
  ...
  @c mysql_Mutex is a drop-in replacement for @c pthread_mutex_t.
...
*/
typedef struct st_mysql_mutex mysql_Mutex;
...
/**
  @def mysql_Mutexrylock(M)
  Instrumented mutex_lock.
  @c mysql_Mutexrylock is a drop-in replacement
  for @c pthread_mutex_trylock.
*/

#if defined(SAFE_MUTEX) || defined (HAVE_PSI_MUTEX_INTERFACE)
  #define mysql_Mutexrylock(M) \
    inline_mysql_Mutexrylock(M, __FILE__, __LINE__)
#else
  #define mysql_Mutexrylock(M) \
    inline_mysql_Mutexrylock(M)
#endif
...
static inline int inline_mysql_Mutexrylock(
  mysql_Mutex *that
#if defined(SAFE_MUTEX) || defined (HAVE_PSI_MUTEX_INTERFACE)
  , const char *src_file, uint src_line
#endif
